...
requests["Find the actors with all information who played a role in the movies 'Annie Hall'"] = "SELECT actors.id , act_fname, act_lname, act_gender FROM actors INNER JOIN movies_actors ON actors.id = movies_actors.act_id INNER JOIN movies ON movies.id = movies_actors.mov_id WHERE mov_title ='Annie Hall';"
requests["Find the first and last names of all the actors who were cast in the movies 'Annie Hall', and the roles they played in that production"] = "SELECT act_fname,act_lname,role FROM actors JOIN movies_actors ON actors.id=movies_actors.act_id JOIN movies ON movies.id=movies_actors.mov_id  WHERE mov_title ='Annie Hall';"
requests["Find the name of movie and director who directed a movies that casted a role as Sean Maguire"] =  "SELECT dir_fname, dir_lname, mov_title FROM movies_actors JOIN movies ON movies.id = movies_actors.mov_id JOIN directors_movies ON movies.id = directors_movies.mov_id JOIN directors ON directors_movies.dir_id = directors.id WHERE role = 'Sean Maguire';"
requests["Find all the actors who have not acted in any movie between 1990 and 2000 (select only actor first name, last name, movie title and release year)"] =  "SELECT a.act_fname, a.act_lname, m.mov_title, CAST(strftime('%Y', m.mov_dt_rel) AS INTEGER) AS release_year
FROM actors AS a
LEFT JOIN movies_actors AS ma ON a.id = ma.act_id
LEFT JOIN movies AS m ON ma.mov_id = m.id
WHERE a.id NOT IN (
    SELECT ma.act_id
    FROM movies_actors AS ma
    JOIN movies AS m ON ma.mov_id = m.id
    WHERE CAST(strftime('%Y', m.mov_dt_rel) AS INTEGER) BETWEEN 1990 AND 2000
)
AND m.mov_title IS NOT NULL;" 